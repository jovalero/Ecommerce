...
async function run() {
  ...
  for await (const line of rl) {
    if (line.includes('USER_INPUT') && line.includes('hero_slides')) {
  ...
  if (!targetContent) {
    console.log('No user input with hero_slides found');
    return;
  }

  console.log('Total content length:', targetContent.length);

  const firstBrace = targetContent.indexOf('{');
  ...
  const jsonStr = targetContent.substring(firstBrace, lastBrace + 1);
  ...
  if (Array.isArray(configData.hero_slides)) {
    ...
    configData.hero_slides.forEach((slide, i) => {
      console.log(`Hero slide [${i}]: title="${slide.title}", span="${slide.span}", cta="${slide.cta}"`);
  ...
  console.log('Successfully saved config to src/config/initialStoreData.js');
}
